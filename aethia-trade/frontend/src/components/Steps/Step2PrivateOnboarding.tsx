...
export default function Step2PrivateOnboarding({ userProfile, setUserProfile, onNext }: Step2Props) {
  ...
  const handleFinishOnboarding = async () => {
    try {
      const response = await fetch("http://localhost:8000/api/profile/behavioral", {
        method: "POST",
      ...
      if (response.ok) {
        ...
        // Fallback local calculations if API is unavailable
        const risk = userProfile.riskTolerance || 5;
        const isPreservation = userProfile.investmentGoal.includes("Preservation");
      ...
      console.error("API Error generating profile", error);
      // Fallback local calculations if API throws
      const risk = userProfile.riskTolerance || 5;
      const isPreservation = userProfile.investmentGoal.includes("Preservation");
      const anomalyScore = 0.08 + (10 - risk) * 0.01;
      const lossAversionIndex = Math.max(1, 10 - risk + (isPreservation ? 2 : 0));
