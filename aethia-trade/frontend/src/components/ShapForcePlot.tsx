...
export default function ShapForcePlot({ shapFeatures = [] }: ShapForcePlotProps) {
  const baseValue = 10.0;
  
  // Safe calculation fallback if features list is empty
  const totalShapOffset = shapFeatures.length > 0 
    ? shapFeatures.reduce((acc, curr) => acc + (curr.shapValue || 0), 0)
    : 3.6;
  const finalExpectedReturn = baseValue + totalShapOffset;

  // Setup default display items if shapFeatures is not yet loaded
  const displayFeatures = shapFeatures.length > 0 ? shapFeatures : [
    { feature: "LSTM 180-Day Momentum Signal", description: "Deep neural network momentum projection", value: "+12.4% Momentum", shapValue: +2.8, category: "Macro" },
    { feature: "FinBERT Executive Sentiment", description: "NLP sentiment score", value: "0.78 Bullish", shapValue: +1.6, category: "Sentiment" },
    { feature: "US 10Y Real Yield Premium", description: "Inflation-adjusted Treasury yield buffer", value: "2.10% Real Yield", shapValue: -0.9, category: "Valuation" },
  ];
                ...
                <div
                  ...
                  className={`h-5 px-2 rounded flex items-center justify-between text-white font-semibold transition-all ${
                    isPositive
                      ...
                      : "bg-crimson-600 border border-crimson-750"
                  }`}
                  style={{ flexGrow: Math.max(0.1, Math.abs(item.shapValue)) }}
                >
